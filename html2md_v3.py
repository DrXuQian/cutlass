#!/usr/bin/env python3
"""Convert Hexo HTML blog posts to Markdown for Obsidian - Version 3."""
...
def html_to_markdown(html: str) -> str:
    ...
    # Convert headers - handle headers with anchors
    ...
    def convert_code_block(match):
        ...
        # The actual code is in the second td (after gutter)
        # Find all code spans in the code column
        code_col_match = re.search(r'<td class="code"><pre>(.*?)</pre></td>', table_html, re.DOTALL)
        if not code_col_match:
            return ''

        code_html = code_col_match.group(1)

        # Extract code lines - they're in span.line tags
        lines = []
        for line_match in re.finditer(r'<span class="line">(.*?)</span>', code_html, re.DOTALL):
    ...
    md = re.sub(
        r'<figure class="highlight ([^"]+)"[^>]*><table>(.*?)</table></figure>',
        convert_code_block,
        md, flags=re.DOTALL
    )
