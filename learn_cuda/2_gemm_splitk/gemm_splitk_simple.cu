...
int main(int argc, char** argv) {
    ...
    std::cout << "CUTLASS Split-K GEMM Example (Universal API)" << std::endl;
    ...
    // Define CUTLASS GEMM types using GemmUniversal (supports Split-K)
    using RowMajor = cutlass::layout::RowMajor;

    // GemmUniversal supports both regular and Split-K modes
    ...
    // ========================================
    // 1. Regular GEMM (baseline, split_k = 1)
    // ========================================
    std::cout << "\n1. Regular GEMM (no Split-K):" << std::endl;

    CutlassGemm::Arguments args_regular(
    ...
    // Warmup
    cutlass::Status status = gemm_op(args_regular);
    if (status != cutlass::Status::kSuccess) {
        std::cerr << "Regular GEMM failed: " << cutlass::cutlassGetStatusString(status) << std::endl;
        return -1;
    }
    ...
    // ========================================
    // 2. Split-K GEMM using GemmUniversal
    // ========================================
    std::cout << "\n2. Split-K GEMM (" << split_k_slices << " slices):" << std::endl;

    // For Split-K mode, use kGemmSplitKParallel
    CutlassGemm::Arguments args_splitk(
        cutlass::gemm::GemmUniversalMode::kGemmSplitKParallel,  // Split-K mode
        {M, N, K},           // Problem size
    ...
    // Warmup
    status = gemm_op(args_splitk);
    if (status != cutlass::Status::kSuccess) {
        std::cerr << "Split-K GEMM failed: " << cutlass::cutlassGetStatusString(status) << std::endl;
        return -1;
    }
    CHECK_CUDA(cudaDeviceSynchronize());
    ...
    for (int slices : slice_counts) {
        ...
        cutlass::gemm::GemmUniversalMode mode = (slices == 1) ?
            cutlass::gemm::GemmUniversalMode::kGemm :
            cutlass::gemm::GemmUniversalMode::kGemmSplitKParallel;

        CutlassGemm::Arguments args_test(
            mode,
            {M, N, K},
            slices,
            {alpha, beta},
            d_A, d_B, d_C, d_D_splitk,
            M * K, K * N, M * N, M * N,
            K, N, N, N
        );
    ...
    std::cout << "Split-K benefit: " << std::setprecision(2)
              << ((regular_time/best_time - 1) * 100) << "% improvement" << std::endl;

    if (best_slices > 1 && regular_time/best_time > 1.05) {
